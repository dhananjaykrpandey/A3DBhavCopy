...
{
    public partial class FrmBhavCopyData : RadForm
    {
        ...
        private void FrmBhavCopyData_Load(object sender, EventArgs e)
        {
            GetCompaniesDetails();
        }
        ...
        private bool LValidateFilter()
        {
            try
            {
                ...
                {
                    ClsMessage._IClsMessage.ProjectExceptionMessage("From Date Cannot Be Grater Than To Date");
                    RdDtpFrom.Focus();
                    LResult = false;
                }
                ...
                else if (LstRdCheckBoxes.Count < 0)
                {
                    ClsMessage._IClsMessage.ProjectExceptionMessage("Please Set Bhav-Copy Required Columns");
                    RdChkOpen.Focus();
                    LResult = false;
                }
                return LResult;
        ...
        private void RdChkOpen_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            try
            {
                ...
                {
                    if (LstRdCheckBoxes.Contains(_RdCheckBox) == false)
                    {
        ...
        private void PopulateData()
        {
            try
            {
                ...
                for (int i = 0; i <= _IDaysInMonth; i++)
                {
                    ...
                    gridViewColumnGroup.Rows.Add(new GridViewColumnGroupRow());
                    foreach (var _RdCheckBox in LstRdCheckBoxes)
                    {
                        DcColBhavCopyData = new DataColumn(string.Concat(_RdCheckBox.Tag.ToString(), "_", _DtMonthDate.ToString("dd-MMM-yyyy")), typeof(double));
                        DcColBhavCopyData.DefaultValue = 0;
                        DcColBhavCopyData.Caption = _RdCheckBox.Text;
                        DtBhavCopyData.Columns.Add(DcColBhavCopyData);

                        gridViewColumnGroup.Rows[0].ColumnNames.Add(string.Concat(_RdCheckBox.Tag.ToString(), "_", _DtMonthDate.ToString("dd-MMM-yyyy")));
                    }
                    ...
                    DcColBhavCopyData = new DataColumn("cPriceOpen_" + _DtMonthDate.ToString("dd-MMM-yyyy"), typeof(double));
                    DcColBhavCopyData.DefaultValue = 0;
                    DcColBhavCopyData.Caption = "Opening Price";
                    DtBhavCopyData.Columns.Add(DcColBhavCopyData);


                    DcColBhavCopyData = new DataColumn("cPriceCLose_" + _DtMonthDate.ToString("dd-MMM-yyyy"), typeof(double));
                    DcColBhavCopyData.DefaultValue = 0;
                    DcColBhavCopyData.Caption = "Closing Price";
                    DtBhavCopyData.Columns.Add(DcColBhavCopyData);
                ...
                foreach (DataRowView DrvCompany in DtBhavCopyData.DefaultView)
                {
                    ...
                    foreach (DataRowView DrvBhavCopySqlData in DtBhavCopySqlData.DefaultView)
                    {
                        foreach (var _RdCheckBox in LstRdCheckBoxes)
                        {
                            if (DtBhavCopyData.Columns.Contains(string.Concat(_RdCheckBox.Tag.ToString(), "_", Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy"))))
                            {
                                DrvCompany[string.Concat(_RdCheckBox.Tag.ToString(), "_", Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy"))] = DrvBhavCopySqlData[_RdCheckBox.Tag.ToString()];

                            }
                        }

                        if (DtBhavCopyData.Columns.Contains("cPriceOpen_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")))
                        {
                            DrvCompany["cPriceOpen_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")] = Math.Round(Convert.ToDouble(DrvBhavCopySqlData["cOPEN"]), 2);

                        }

                        if (DtBhavCopyData.Columns.Contains("cPriceCLose_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")))
                        {
                            DrvCompany["cPriceCLose_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")] = Math.Round(Convert.ToDouble(DrvBhavCopySqlData["cCLOSE"]), 2);

                        }

                        if (DtBhavCopyData.Columns.Contains("cPriceChange_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")))
                        {
                            DrvCompany["cPriceChange_" + Convert.ToDateTime(DrvBhavCopySqlData["dTIMESTAMP"]).ToString("dd-MMM-yyyy")] = Math.Round(Convert.ToDouble(DrvBhavCopySqlData["cCLOSE"]) - Convert.ToDouble(DrvBhavCopySqlData["cOPEN"]), 2);
